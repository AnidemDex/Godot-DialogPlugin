...
func _ready() -> void:
	...
	timeline_container_node.tree_resource = DialogDB.Timelines.get_database()
	character_container_node.tree_resource = DialogDB.Characters.get_database()
...
func _set_current_view(view:Dictionary):
	...
	DialogUtil.Logger.print(self,["Changing view to:", _current_view["reference"].name])
...
func _on_NewCharacterPopup_confirmed() -> void:
	DialogUtil.Logger.print(self,["NewCharacter: ", character_popup_node.text_node.text])
	var _name = character_popup_node.text_node.text
	DialogDB.Characters.add(_name)
...
func _on_TimelinesContainer_tree_item_selected(tree_item:TreeItem) -> void:
	...
	if _res:
		DialogUtil.Logger.print(self,["Using resource:", _res.resource_path])
		self._current_view = {"state":EditorView.TIMELINE, "reference":timeline_view_node}
	else:
		self._current_view = {}
...
func _on_CharactersContainer_tree_item_selected(tree_item) -> void:
	...
	if _res:
		DialogUtil.Logger.print(self,["Using character:", _res.resource_path])
		self._current_view = {"state": EditorView.CHARACTER, "reference":character_view_node}
	else:
		self._current_view = {}
