...
func _update_node_values() -> void:
	time_line_node.text = str((base_resource as DialogWaitTimeEvent).wait_time)
...
func _on_LineEdit_text_changed(new_text: String) -> void:
	...
	if _time <= 0 or _time > 60*60:
		DialogUtil.Logger.print(self, "Invalid time")
		_time = 0.0
	
	if _time != base_resource.wait_time:
		(base_resource as DialogWaitTimeEvent).wait_time = _time
